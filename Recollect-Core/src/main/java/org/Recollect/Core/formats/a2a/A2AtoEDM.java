 ...
 * 
 */
package org.Recollect.Core.formats.a2a;
...
import org.Recollect.Core.serialize.JibxMarshall;
...
public class A2AtoEDM {
	 ...
	 * @param outs
	 */
	public A2AtoEDM(String identifier, A2AType a2atype, Map<String, String> properties, OutputStream outs) {
	...
	public void marshal() {
		if (Objects.nonNull(rdf) && !Objects.equals(rdf, new RDF()))
			JibxMarshall.marshall(rdf, "UTF-8", true, out, RDF.class);
	}
	...
	private void oreAggregation() {
		Optional.ofNullable(a2a.getSource()).ifPresent(present -> {
			...
			Optional.of(properties).map(m -> m.get("rights")).ifPresent(data -> {
				...
				Resource resource = new Resource();
				resource.setResource(IriToUri.iriToUri(data).toString());
				rights.setResource(resource);
				
				aggregation.getRightList().add(rights);
			});

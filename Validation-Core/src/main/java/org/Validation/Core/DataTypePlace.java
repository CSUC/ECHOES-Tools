...
import org.Validation.Core.edm.PlaceTypes;

/**
 * @author amartinez
 */
public class DataTypePlace {

    private static Logger logger = LogManager.getLogger(DataTypePlace.class);
    private List<RDF.Choice> listChoice;

    public DataTypePlace(List<RDF.Choice> listChoice) {
    ...
    protected boolean placeType(String value) {
        PlaceTypes placeTypes = Objects.nonNull(value) ? PlaceTypes.convert(value) : null;
        boolean match = Objects.nonNull(placeTypes);
        logger.debug("[placeType]      \"{}\"     validation", match ? placeTypes.xmlValue() : null, match);
        return match;
    }

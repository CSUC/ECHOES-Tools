...
<xsl:stylesheet 
	...
	<xsl:template match="@*|node()">
		<xsl:for-each select="/ns1:OAI-PMH/ns1:ListRecords/ns1:record">			
			<xsl:result-document method="xml" href="{$folder}/{$collectionSet}/{$dir}/file{position()}.xml">
				<rdf:RDF xmlns:dc="http://purl.org/dc/elements/1.1/"
					...
					<xsl:if test="./ns1:metadata/a2a:A2A/a2a:Source/a2a:SourceAvailableScans/a2a:Scan/a2a:UriViewer != ''">
						<xsl:for-each select="./ns1:metadata/a2a:A2A/a2a:Source/a2a:SourceAvailableScans">
							<ore:Aggregation>
							 	<xsl:attribute name="rdf:about">
							 		<xsl:variable name="AggregationAbout" select="./a2a:Scan/a2a:UriViewer/text()"/>
									<xsl:value-of select="replace($AggregationAbout,'\s','_')"/>
